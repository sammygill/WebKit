...
bool RenderBox::shouldComputePreferredLogicalWidthsFromStyle() const
{
    if (isRenderBlock()) {
        ...
        return !isRenderTableCell() && logicalWidthLength.isFixed() && logicalWidthLength.value() >= 0 && !(isDeprecatedFlexItem() && !logicalWidthLength.intValue());
    }
    auto logicalWidthLength = style().logicalWidth();
    return logicalWidthLength.isFixed() && logicalWidthLength.value();
}
...
void RenderBox::computePreferredLogicalWidths(const Length& minLogicalWidth, const Length& maxLogicalWidth, LayoutUnit borderAndPaddingLogicalWidth)
{
    ...
    auto usedMaxLogicalWidth = [&] {
        ...
        if (maxLogicalWidth.isMinContent()) {
            ...
            static constexpr LayoutUnit s_noBorderAndPaddingLogicalWidth;
            return computeIntrinsicLogicalWidthUsing(maxLogicalWidth, availableLogicalWidth(), s_noBorderAndPaddingLogicalWidth);
        }
